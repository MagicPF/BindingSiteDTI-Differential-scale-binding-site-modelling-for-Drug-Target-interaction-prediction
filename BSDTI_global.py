...
from GAT_layer import GraphAttentionConv, AttentionReadout
...
setting = f"BSDTI3-{set_name}-d={d_layer}-p={p_layer}-bs={bs_layer}-d_seg={d_seg}-k={K}-TD={TD}-T_dim={T_dim}-T_dp={T_dropout}-head={heads}-sim={sim}-epc={epochs}-bth={batch_size}-lr={lr}-rep={rep_d}-mlp={mlp_layer}-pnd={p_nodes[:bs_layer]}-least={least}-seed={opt.seed}"
...
class DrugEncoder(nn.Module):
    ...
    def forward(self,d_graph, d_input):
        ...
        for layer in self.d_gat:
            ...
            d_hid.append(hidden)
            layer_input = hidden
        global_representation = self.attn_pool(d_graph,hidden)
        d_graph, d_hid[-1],_ = self.d_pool(d_graph, d_hid[-1])
        return d_hid[-1].view(b_size, d_seg, rep_d),global_representation

class MMCMAL(nn.Module):

    def __init__(self,in_dim, p_node, out_dim=rep_d, h_layer=1,num_head=1,batch_norm=True,need_rep=False):
        super(MMCMAL, self).__init__()
    ...
    def forward(self,drug_ref, p_graph, layer_input, groundtruth):
        ...
        for layer in self.p_gat:
            ...
            layer_input = hidden
        if self.need_rep:
            global_representation = self.attn_pool(p_graph,layer_input)
        ...
        if self.need_rep:
            return protein_rep, p_graph, hidden.reshape(b_size*self.p_node,rep_d), closs,global_representation
        return protein_rep, p_graph, hidden.reshape(b_size*self.p_node,rep_d), closs,None


class BindingSiteNetwork(nn.Module):

    def __init__(self, d_dim=67,p_dim=64,num_head=4, rep_dim=rep_d):
        super(BindingSiteNetwork, self).__init__()
        self.drug_encoder = DrugEncoder(d_dim=d_dim, negative_slope=0.2, batch_norm=True, rep_dim=rep_dim)
        self.MMCMALs = nn.ModuleList()
        self.MMCMALs.append(MMCMAL(p_dim, p_nodes[0], out_dim=rep_dim, h_layer=p_layer, num_head=num_head,batch_norm=True,need_rep=True))
        for i in range(1, bs_layer):
            self.MMCMALs.append(MMCMAL(rep_dim, p_nodes[i], out_dim=rep_dim, h_layer=0, num_head=num_head, batch_norm=True,need_rep=(i==1)))
    ...
    def forward(self, d_graph, d_input, p_graph, p_input, groundtruth=None):
        drug_feature,drug_global = self.drug_encoder(d_graph, d_input)
        protein_segs = None
        complementary_loss = None
        protein_global = None
        for index, layer in enumerate(self.MMCMALs):
            segs, p_graph, p_input, c_loss, pg = layer(drug_feature, p_graph, p_input, groundtruth)
            if index==0:
                protein_global = pg
            if protein_segs is None:
                ...
                if c_loss is not None and c_loss.item() > complementary_loss.item():
                    complementary_loss = c_loss
        # st()
        interaction = self.decoder(torch.cat((protein_segs,protein_global.unsqueeze(dim=1)),dim=1),torch.cat((drug_feature,drug_global.unsqueeze(dim=1)),dim=1))
        weights = self.softmax(interaction)
        interaction = (weights * interaction)
        return interaction, complementary_loss


class DTIDNN(nn.Module):
    def __init__(self, d_dim=67,p_dim=64, out_dim=2, rep_dim=rep_d):
        super().__init__()
        self.out_dim = out_dim
        self.rep_dim = rep_dim
        self.bsn = BindingSiteNetwork(d_dim=d_dim, p_dim=p_dim, num_head=heads, rep_dim=rep_d)
        self.feature_num = 0
        for bs in range(bs_layer):
            self.feature_num += math.ceil(p_nodes[bs] * K) * rep_d
        self.mlp = MultiLayerPerceptron(self.feature_num+rep_d, [rep_d*2]*mlp_layer + [rep_d], short_cut=False, batch_norm=True, activation='relu',dropout=0)
...
def load_dataset_model(dataset_name,train_file_name="train",val_file_name="valid",test_file_name="test"):
    if  "human" in dataset_name:
        ...
        model = DTIDNN(d_dim=67,p_dim=21, out_dim=2) 
